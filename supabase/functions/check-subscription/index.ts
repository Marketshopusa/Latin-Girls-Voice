...
// Product IDs mapping to plan tiers
...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      ...
      return new Response(JSON.stringify({ subscribed: false, plan: 'free', product_id: null, subscription_end: null }), {
    ...
    if (hasActiveSub) {
      ...
      plan = PLAN_TIERS[productId] || 'free';
